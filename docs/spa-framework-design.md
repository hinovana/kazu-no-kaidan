...
| --- | --- |
| 文書状態 | 設計草案 |
| 文書版 | `spa-framework-design.v0.1-draft` |
| 最終更新日 | 2026-07-15 |
...
Vite開発サーバーは既存AIプロキシのCORS契約を不用意に変えないため、初期値を `127.0.0.1:8765` とする。ポート変更を許す場合は、AIプロキシのallowlistも明示的に同期する。
...
- 現行ルート一覧と3教材のURLを記録する。
- 各画面と印刷の基準画像・PDFまたは検査結果を保存する。
- `npm test` と各コーパステストの成功を確認する。

### フェーズ1：ホスト基盤

- Vite、TypeScript、React、React Router、Vitestを導入する。
- ルートSPA、Hash Router、共通シェル、レジストリを実装する。
- 既存教材をlegacy entryとして一覧へ登録する。
- Vite buildへ既存HTMLを複数入力として追加する。

### フェーズ2：こくごのたねdomain移行

- [こくごのたね TypeScriptモジュール化設計書](../generators/kokugo-no-tane/docs/typescript-module-design.md) に従う。
- 既存JS版と新TS版を差分比較できる期間を設ける。

### フェーズ3：こくごのたねUI移行

- React UIをSPA moduleとして登録する。
- UI、印刷、AIプロキシ接続、フォールバックを確認する。
- 完了条件を満たした後だけ旧URLを互換転送へ切り替える。
