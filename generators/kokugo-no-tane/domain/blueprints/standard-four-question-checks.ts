...
export function runStandardFourQuestionChecks(
  ...
  const extracts: readonly Question[] = [worksheet.questions[0]!, worksheet.questions[2]!];
  const q2 = worksheet.questions[1] as EmotionChoiceQuestion;
  const q4 = worksheet.questions[3] as InferEmotionQuestion;
  const q4EvidencePositions = q4.evidence_ids.map((id) =>
    worksheet.passage.sentences.findIndex((sentence) => sentence.sentence_id === id));
  const q4EvidenceDistance = (q4EvidencePositions[1] ?? -1) - (q4EvidencePositions[0] ?? -1);
  const minimumQ4EvidenceDistance = [1, 1, 2, 3, 5][worksheet.generation_profile - 1] ?? 1;
