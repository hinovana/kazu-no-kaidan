...
export function getKnownKanjiSet(grade) {
  if (![1, 2, 3].includes(grade)) {
    throw new RangeError("grade must be 1, 2, or 3");
  }
  if (!CUMULATIVE.has(grade)) {
    CUMULATIVE.set(
      grade,
      new Set(Array.from({ length: grade }, (_, index) => BY_GRADE[index + 1]).join("")),
    );
  }
  return new Set(CUMULATIVE.get(grade));
}

export function isKnownOrthography(surface, grade) {
