...
export function validateStoryPlanRequest(value) {
  const issues = [];
  ...
  if (!Number.isInteger(value.grade) || value.grade < 1 || value.grade > 3) issues.push("grade must be 1, 2, or 3");
  if (!Number.isInteger(value.profile) || value.profile < 1 || value.profile > 5) issues.push("profile must be 1 through 5");
  if (!LENGTHS.includes(value.length)) issues.push("length is not supported");
  if (!TOPICS.includes(value.topic)) issues.push("topic is not supported");
...
export function validateStoryPlan(value) {
  const issues = [];
  ...
  if (!STORY_CATEGORIES.includes(value.category)) issues.push("category is not supported");
  ...
  if (isRecord(value.setting)) {
    if (!SETTING_TYPES.includes(value.setting.type)) issues.push("setting.type is not supported");
...
export function parseStoryPlan(value) {
  const issues = validateStoryPlan(value);
  if (issues.length > 0) {
    const error = new TypeError(`invalid story plan: ${issues.join("; ")}`);
...
export function expectedCategoryForTopic(topic) {
  return ({ school: "学校", home: "家庭", nature: "自然", town: "町", animal: "動物" })[topic] ?? null;
}
...
function person(value, label, issues) {
...
function readerText(value, label, issues) {
...
function objectWithKeys(value, keys, label, issues) {
  if (!isRecord(value)) {
