...
} from "./generated/prototype-vocabulary-evidence.js";
...
export function buildVocabularyAudit(rubyPlan) {
  const occurrences = rubyPlan.map((occurrence) => {
    const evidence = PROTOTYPE_VOCABULARY_EVIDENCE[occurrence.lexeme_id] ?? null;
    return {
...
export function validateVocabularyAudit(worksheet) {
  const issues = [];
  ...
  audit.occurrences.forEach((occurrence, index) => {
    ...
    const evidence = PROTOTYPE_VOCABULARY_EVIDENCE[occurrence.lexeme_id];
