...
import { requestCodexStoryPlan } from "./codex-story-plan.ts";
import {
  loadAiProxyConfig,
  type AiProxyConfig,
} from "./config.ts";
import { createJsonlFileLogger } from "./file-logger.ts";
import {
  requestOpenAiStoryPlan,
...
type Logger = (value: Readonly<Record<string, unknown>>) => void;
type StoryPlanGenerator = (input: {
  readonly request: StoryPlanProviderRequest;
  readonly signal: AbortSignal;
}) => Promise<StoryPlanProviderResult>;

interface CreateAiProxyServerOptions {
  readonly config: Readonly<AiProxyConfig>;
  readonly generateStoryPlan?: StoryPlanGenerator;
  readonly openaiClient?: OpenAiClientLike;
  readonly candidateStore?: CandidateStore;
  readonly logger?: Logger;
  readonly providerLogger?: ProviderLogger;
}
...
interface RequestLogValue {
  ...
  readonly body?: StoryPlanProviderRequest;
  readonly status: number;
  readonly startedAt: number;
  readonly cacheHit?: boolean;
  readonly candidateId?: unknown;
  readonly providerResponseId?: unknown;
...
export function createAiProxyServer(
  options: Partial<CreateAiProxyServerOptions> = {},
): Server {
  const config = options.config;
  if (!config) throw new TypeError("config is required");
  const candidateStore = options.candidateStore ?? createCandidateStore(config.localDir);
  const logger = options.logger ?? defaultLogger;
  const providerLogger = options.providerLogger ?? logger;
  const { generateStoryPlan, openaiClient } = options;
  const storyPlanGenerator = generateStoryPlan ?? (openaiClient
    ? ({ request, signal }) => requestOpenAiStoryPlan({
      ...
      request,
      signal,
      logger: config.logIo ? providerLogger : undefined,
    })
    : createStoryPlanGenerator(config, config.logIo ? providerLogger : undefined));
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      const validatedBody = body as StoryPlanProviderRequest;
      ...
      if (cached) {
        const payload = {
          ...cached,
          request_id: requestId,
          client_request_id: validatedBody.client_request_id,
          cache: { hit: true },
        };
        sendJson(response, 200, payload, cors ?? {});
        logResult(logger, { requestId, body: validatedBody, status: 200, startedAt, cacheHit: true, candidateId: cached.candidate_id });
        return;
      }

      const result = await callWithRetry({ config, generateStoryPlan: storyPlanGenerator, request: validatedBody });
...
function createStoryPlanGenerator(
  config: Readonly<AiProxyConfig>,
  logger: ProviderLogger,
): StoryPlanGenerator {
  ...
  const openaiClient = new OpenAI({ apiKey: config.apiKey }) as unknown as OpenAiClientLike;
  ...
  for (let attempt = 0; attempt <= config.maxRetries; attempt += 1) {
    ...
    try {
      ...
      if (controller.signal.aborted) {
        throw Object.assign(new Error("AI provider request timed out"), { code: "AI_TIMEOUT" });
      }
...
function normalizeError(cause: unknown): ProxyError {
  if (isProxyError(cause)) return cause;
  const code = errorProperty(cause, "code");
  const name = errorProperty(cause, "name");
  const status = Number(errorProperty(cause, "status"));
  if (code === "AI_REFUSAL") return proxyError("AI_REFUSAL", 502, "物語設計図を作成できませんでした。", true);
  if (code === "SCHEMA_INVALID") return proxyError("SCHEMA_INVALID", 502, "AIの物語設計図を検証できませんでした。", true);
  if (code === "CONTENT_REJECTED") return proxyError("CONTENT_REJECTED", 502, "AIの物語設計図が教材条件を満たしませんでした。", true);
  if (code === "AI_TIMEOUT" || name === "AbortError") return proxyError("AI_TIMEOUT", 504, "物語設計図の取得が時間内に終わりませんでした。", true);
  if (code === "AI_AUTH_FAILED") return proxyError("AI_AUTH_FAILED", 502, "AIサーバーの認証を確認してください。", true);
  if (code === "AI_UNAVAILABLE") return proxyError("AI_UNAVAILABLE", 503, "AIサービスを利用できません。", true);
  if (status === 401 || status === 403) return proxyError("AI_AUTH_FAILED", 502, "AIサーバーの認証を確認してください。", true);
  if (code === "insufficient_quota") {
    return proxyError("AI_QUOTA_EXCEEDED", 429, "OpenAI APIの利用枠または請求設定を確認してください。", true);
  }
  if (status === 429) return proxyError("AI_RATE_LIMITED", 429, "AIサービスの呼び出し上限に達しました。", true);
  if (status >= 500) return proxyError("AI_UNAVAILABLE", 503, "AIサービスを利用できません。", true);
...
function serializableProviderResponse(response: unknown): Readonly<Record<string, unknown>> {
...
function logResult(logger: Logger, value: RequestLogValue): void {
  logger({
...
function defaultLogger(value: Readonly<Record<string, unknown>>): void {
  process.stdout.write(`${JSON.stringify(Object.fromEntries(Object.entries(value).filter(([, item]) => item !== undefined)))}\n`);
}

function safeErrorMessage(error: unknown): string {
  return String(errorProperty(error, "message") ?? error ?? "unknown error").slice(0, 240);
}
...
function isProxyError(error: unknown): error is ProxyError {
  return error instanceof Error
    ...
    && typeof errorProperty(error, "publicMessage") === "string"
    && typeof errorProperty(error, "code") === "string";
}
