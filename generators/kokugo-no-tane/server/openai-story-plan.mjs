...
import { buildStoryPlanContext } from "./story-plan-context.mjs";
...
export function parseGeneratedStoryPlan(outputText, request) {
  ...
  try {
    ...
    const error = new Error("The structured output was not valid JSON");
    error.code = "SCHEMA_INVALID";
    throw error;
  }
  ...
  if (expectedCategory && storyPlan.category !== expectedCategory) {
    const error = new Error("The story category did not match the requested topic");
