...
export type AiProviderName = "codex" | "openai";
...
export interface AiProxyConfig {
  ...
  readonly logIo: boolean;
  readonly localDir: string;
  readonly requestsPerMinute: number;
}

export function loadAiProxyConfig(
  env: Readonly<Record<string, string | undefined>> = process.env,
): Readonly<AiProxyConfig> {
  ...
  const config = {
    ...
    allowedOrigins: splitOrigins(
      env.KNT_FRONTEND_ORIGINS || "http://127.0.0.1:5173,http://127.0.0.1:8765",
    ),
  ...
  return Object.freeze(config as AiProxyConfig);
}
...
function providerName(value: unknown): AiProviderName {
  const normalized = String(value).trim().toLowerCase();
  if (["codex", "openai"].includes(normalized)) return normalized as AiProviderName;
...
function isAllowedFrontendOrigin(value: string): boolean {
  try {
...
function boolean(raw: unknown, fallback: boolean, name = "boolean value"): boolean {
  if (raw === undefined || raw === "") return fallback;
...
function configError(message: string): Error & { readonly code: "CONFIG_ERROR" } {
  return Object.assign(new Error(message), { code: "CONFIG_ERROR" as const });
}
