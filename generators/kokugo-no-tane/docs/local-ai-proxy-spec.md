...
| --- | --- |
| 文書状態 | 実装契約 |
| 文書版 | `local-ai-proxy.v0.2` |
| 最終更新日 | 2026-07-15 |
| 実装状態 | 実装済み。モック契約・全体テスト合格、実APIは利用枠不足によりモデル出力未取得 |
| 対象実装 | 開発者のPCで起動するNode.js製AIプロキシと、ブラウザ側の接続UI |
| Node.js | `26.5.0`（`.node-version` と `.nvmrc` で固定） |
| 既定フロントエンド | `http://127.0.0.1:8765/generators/kokugo-no-tane/` |
| 既定プロキシ | `http://127.0.0.1:8787` |
| 外部接続先 | OpenAI Responses API |
...
ブラウザ
  ...
  ↓
ローカルAIプロキシ（Node.js）
  │ OPENAI_API_KEYを環境変数から読む
  │ 入力検証・レート制限・キャッシュ・候補保存
  ↓
OpenAI Responses API
  │ Structured Outputsによるstory-plan.v1
  ↓
プロキシ側のスキーマ・内容検査
  ↓
ブラウザ側の再検査
  ↓
既存アルゴリズムが本文・4問・解答・根拠を生成
```
...
| 層 | 責務 | 行わないこと |
| --- | --- | --- |
| ブラウザ | 条件入力、接続状態表示、プロキシ呼出し、レスポンス再検査、問題生成 | APIキーの保持、OpenAI APIへの直接接続 |
| ローカルプロキシ | 秘密情報、入力検証、OpenAI呼出し、出力検査、候補保存、キャッシュ、CORS | 最終本文・設問・正答の決定 |
| OpenAI API | `story-plan.v1` 候補の生成 | 公開可否、正答一意性、学年適合性の最終判断 |
| 決定的ジェネレータ | story planから本文・4問・正答・根拠を生成 | AI出力への無条件の信頼 |
...
APIキーをコマンド引数、URL、ブラウザフォームへ渡さない。シェル履歴へ残る可能性があるため、標準手順では `OPENAI_API_KEY=... npm run serve:ai` のインライン指定を案内しない。

### 3.3 環境変数

| 変数 | 必須 | 既定値 | 用途 |
| --- | --- | --- | --- |
| `OPENAI_API_KEY` | 必須 | なし | OpenAI API認証。ログ・レスポンスへ出さない |
| `OPENAI_MODEL` | 任意 | `gpt-5.6` | モデルIDまたはスナップショットID。再現性が必要な評価では明示的に固定する |
| `KNT_AI_HOST` | 任意 | `127.0.0.1` | bind先。初期実装ではloopback以外を拒否する |
| `KNT_AI_PORT` | 任意 | `8787` | プロキシの待受ポート |
| `KNT_FRONTEND_ORIGINS` | 任意 | `http://127.0.0.1:8765` | CORSで許可する完全一致Originのカンマ区切り |
| `KNT_AI_TIMEOUT_MS` | 任意 | `45000` | OpenAI API呼出しの上限時間 |
| `KNT_AI_MAX_RETRIES` | 任意 | `1` | 一時障害時の追加試行回数 |
| `KNT_AI_SAVE_CANDIDATES` | 任意 | `1` | 合格候補をローカル保存するか |

必須変数がない場合は、待受を開始せず、変数名だけを含むエラーを標準エラーへ出して終了コード1で停止する。値そのものは表示しない。
...
| 生成条件プロファイル | 送る | AIの構成ヒントに使い、既存generatorが問4の根拠間距離を確定する |
...
seedの「ランダム」操作は入力値を変更するだけであり、生成やAPI通信を開始しない。API通信は「この条件でつくる」の明示操作でのみ開始する。
...
OpenAI APIへ接続せず、プロキシの起動状態と公開可能な設定だけを返す。APIキーの値、先頭・末尾文字、ハッシュを返してはならない。
...
- 文科省由来の学年別配当漢字候補、正式語彙DBが未実装であること、限定 `prototype_lexicon`、参照アンカー12・18から抽出した読み支援の抽象特徴を `story-plan-context.v1` として送る。
- 漢字候補は下流の表記検査用であり、かな限定のstory planへ漢字を混入させる目的では使わない。未実装の語彙DBについて学年適合を保証したとモデルへ伝えない。
- `text.format` のJSON SchemaによるStructured Outputsを使う。
- モデル拒否、途中終了、APIエラー、出力欠落を明示的に分岐する。
- `store: false` を指定する。ただし、これだけでZero Data Retentionになるとは扱わない。
- 初期実装では会話継続、web search、file search、外部MCP、任意ツールを使用しない。
- 生成結果は必ずアプリ側の検証器へ通し、Structured Outputsの適合だけで教育的合格とみなさない。
...
APIキー、生成日時、request IDはキャッシュキーへ含めない。キャッシュに入れるのは検証合格済みstory planだけとし、不合格・拒否・エラーを成功キャッシュへ保存しない。
...
実装時に `.local/` をGit管理対象外へ追加する。自動保存したファイルをそのまま配布データへ昇格させない。保存レコードには、`candidate_id`、モデル、プロンプト版・ハッシュ、スキーマ版、入力条件、OpenAI response ID、取得日時、生レスポンスのSHA-256、正規化済みstory plan、検査結果を含める。APIキーは含めない。
...
生レスポンスは標準ログではなく、前節のGit管理外候補ファイルへアクセス権を限定して保存する。
...
│   ├── ai-proxy.mjs
│   ├── config.mjs
│   ├── openai-story-plan.mjs
│   ├── candidate-store.mjs
│   └── story-plan-context.mjs
├── src/
│   ├── ai-proxy-client.js
│   ├── story-plan-schema.js
│   ├── prototype-lexicon.js
│   └── generator.js
├── tests/
│   ├── ai-proxy.test.js
...
通常の `npm test` は実APIやAPIキーなしで成功しなければならない。OpenAI SDK呼出しを差し替え可能にし、fixtureで次を検証する。

### 12.1 プロキシ

- 必須環境変数の欠落時に秘密値を出さず停止する。
- `/health` がAPIキーを返さない。
...
実APIテストは標準テストから分離し、明示的な環境変数がある場合だけ1件実行する。出力内容を固定一致で検査せず、HTTP成功、schema適合、来歴、候補保存、秘密情報非露出を確認する。利用量が発生することを実行前に表示する。

## 13. 実装完了条件

- `npm run serve:ai` で `127.0.0.1:8787` にNode.jsプロキシが起動する。
- APIキーなしでは安全に起動失敗し、APIキーがHTML、JavaScript、JSON、ログ、Gitへ含まれない。
...
- CORSが `http://127.0.0.1:8765` の完全一致allowlistで動作する。
...
### 13.1 2026年7月15日の検証状況

- Node.js `26.5.0` で単体・契約・UI・ルート全体テストに合格した。
- 実APIテストはResponses APIまで到達し、`insufficient_quota` を受信した。キーの読込と送信経路は確認済みだが、実モデルのschema適合・教材品質・候補保存は未確認である。
- 利用枠を有効化した後、`npm run test:kokugo-no-tane:ai:live` を再実行する。成功時は合格候補を `.local/model-candidates/` へ保存する。
- 実モデルの出力品質を確認するまで、本書の完了条件のうち「AI成功時」の項目は未達とする。
