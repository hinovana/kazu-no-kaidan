...
| --- | --- |
| 文書状態 | 設計草案 |
| 文書版 | `kokugo-no-tane-typescript-module.v0.1-draft` |
| 最終更新日 | 2026-07-15 |
...
export const Page = KokugoNoTanePage;
...
- 現行JS版の全テストを成功させる。
- 代表seedとAI fixtureのJSON出力を保存する。
- ブラウザ画面と印刷結果を保存する。
- 2つのblueprintの自動選択比率を記録する。

### フェーズ1：型とruntime境界

- 基本型、Worksheet、Question、StoryPlanV1を定義する。
- 外部入力parserを追加する。
- JS実装から型付き入口を呼び、まだ処理を変更しない。

### フェーズ2：domain TypeScript化

- random、text renderer、language providerを移行する。
- 共通エンジンを移行する。
- 既存JS版とのdifferential testを毎段階で実行する。

### フェーズ3：blueprintとcontent pack

- 再挑戦型をinterfaceへ適合させる。
- 手がかり発見型をinterfaceへ適合させる。
- scenario、trait、detailsを純粋content packへ移す。
- `pickTrait` とStoryPlan parseを共通エンジンから除去する。

### フェーズ4：applicationとAI infrastructure

- AI proxy clientをport実装へ移す。
- fallback制御をapplication層へ移す。
- NodeプロキシをTypeScript化する。
- browser/serverのprotocol schemaを共有する。

### フェーズ5：React UI

- 現行DOM UIをコンポーネントへ移す。
- reducerで非同期状態を管理する。
- 設問unionと `responseLayout` で紙面を描画する。
- 画面CSSと印刷CSSを移植する。

### フェーズ6：SPA登録

- `module.tsx` をホストregistryへ登録する。
- Hash URL、戻る導線、Error Boundaryを確認する。
- 旧URLは比較期間後に互換転送へ切り替える。

### フェーズ7：旧実装削除

- 全完了条件を満たすまで旧JSを削除しない。
- import、HTML script、テスト入口、仕様リンクの参照を確認する。
- 削除後に全体テスト、build、ブラウザ、印刷を再確認する。
...
- 共通エンジンに `pickTrait()` と固定StoryPlan parseがない。
- AI設計図を非対応blueprintへ渡せない。
- 設問unionと紙面layoutの対応が網羅的である。
- language providerが注入可能で、DB releaseを来歴へ記録する。
- 旧JS版との差分検査で、許可した来歴差以外の問題内容が一致する。
- 1,350件コーパスが成功する。
- UI、A4横・縦書き・2ページ印刷、AI fallbackが確認済みである。
