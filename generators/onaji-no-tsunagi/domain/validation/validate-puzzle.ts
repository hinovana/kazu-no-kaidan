...
export function validatePuzzle(puzzle: Puzzle): ValidationResult {
  ...
  validatePuzzleMetadata(puzzle, issues);
  ...
  validateSymbolCounts(symbolCounts, issues);

  return issues.length === 0 ? {valid: true} : {valid: false, issues};
}

function validatePuzzleMetadata(
  puzzle: Puzzle,
  ...
  if (puzzle.schemaVersion !== 'onaji-no-tsunagi.puzzle.v1') {
    issues.push({
      code: 'schema_version',
      message: '未対応の盤面schemaです。',
    });
  }
  if (!isBoardDimension(puzzle.width)) {
    issues.push({
      code: 'width',
      message: '盤面の幅は2から9の整数で指定します。',
    });
  }
  if (!isBoardDimension(puzzle.height)) {
    issues.push({
      code: 'height',
      message: '盤面の高さは2から9の整数で指定します。',
    });
  }
  if (puzzle.puzzleId.length === 0) {
    issues.push({
      code: 'puzzle_id',
      message: 'puzzleIdは空にできません。',
    });
  }
}

function validateTerminal(
  terminal: Puzzle['terminals'][number],
  puzzle: Puzzle,
  coordinateKeys: Set<string>,
  ...
  if (!SYMBOLS.includes(terminal.symbol)) {
    issues.push({
      code: 'symbol',
      message: `未知の記号です: ${String(terminal.symbol)}`,
    });
  } else {
    symbolCounts.set(
      terminal.symbol,
  ...
  if (coordinateKeys.has(coordinate)) {
    issues.push({
      code: 'terminal_overlap',
      message: `同じマスに複数のマークがあります: ${coordinate}`,
    });
  }
  ...
  if (
    ...
    issues.push({
      code: 'terminal_id',
      message: `terminalIdが空または重複しています: ${terminal.terminalId}`,
    });
  }
  terminalIds.add(terminal.terminalId);
}

function validateSymbolCounts(
  symbolCounts: ReadonlyMap<SymbolId, number>,
  issues: ValidationIssue[],
): void {
...
function isBoardDimension(value: number): boolean {
  return Number.isInteger(value) && value >= 2 && value <= 9;
}
