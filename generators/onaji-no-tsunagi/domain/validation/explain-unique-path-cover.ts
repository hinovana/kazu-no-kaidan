...
export function explainUniquePathCover(
  ...
  if (entry.pairingChoiceCount > 1) {
    const pairingGroups = entry.symbolGroups.filter(
      group => group.pairingChoiceCount > 1,
    );
    witnesses.push({
      kind: 'pairing_choice',
      involvedTerminalIds: pairingGroups.flatMap(group => group.terminalIds),
      temptingConstraint: {
  ...
  witnesses.push({
    ...
    involvedTerminalIds: entry.symbolGroups.flatMap(group => group.terminalIds),
    temptingConstraint: {
